...
public class ExtensionComponent {
	...
	public ExtensionComponent(String categoryId, String creator, String descriptor,
			String id, String name,String ioHandler) {
		super();
	...
	public ComponentCreator getCreator() {
		if (cc == null) {
			try {
				cc = (ComponentCreator) Class.forName(creator)
						.newInstance();
	...
	public DefaultIOHandler getIoHandler() {
		if (dio == null) {
			try {
				dio = (DefaultIOHandler) Class.forName(ioHandler)
						.newInstance();
			} catch (InstantiationException e) {
				DialogUtil.logErrorException(e);
			} catch (IllegalAccessException e) {
