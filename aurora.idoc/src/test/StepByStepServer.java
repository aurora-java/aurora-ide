...
public class StepByStepServer {
	...
	static void step1SimpleServer() {
		...
		server.addServerStateChangedListener(new JCoServerStateChangedListener() {

			@Override
			public void serverStateChangeOccurred(JCoServer arg0,
					JCoServerState arg1, JCoServerState arg2) {

				System.out.println(arg0.getConnectionCount());
	...
	static void step4StaticRepository() {
		...
		impList.add("TRACE_LEVEL", JCoMetaData.TYPE_NUM, 1, 2, 0, null, null,
				JCoListMetaData.IMPORT_PARAMETER, null, null);
		impList.add("TRACE_PATH", JCoMetaData.TYPE_STRING, 8, 8, 0, null, null,
				JCoListMetaData.IMPORT_PARAMETER, null, null);
		impList.lock();
		JCoFunctionTemplate fT = JCo.createFunctionTemplate("JCO_SET_TRACE",
				impList, null, null, null, null);
	...
	static class SetTraceHandler implements JCoServerFunctionHandler {
		public void handleRequest(JCoServerContext serverCtx,
			...
			String path = function.getImportParameterList().getString(
					"TRACE_PATH");
			System.out.println(">>> SetTrace invoked with " + level
					+ (path != null ? (", " + path) : ""));
