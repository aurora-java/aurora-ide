...
import aurora.ide.excel.bank.format.setting.store.Store;
...
public class PreferencesSetting {
	...
	static public void saveXLSSetting(String xls_code, CompositeMap map) {
		...
		Store.saveXLSSetting(xls_code, map);
	}

	static public CompositeMap loadXLSSetting(String xls_code) {
		CompositeMap loadSetting = Store.loadSetting(xls_code);
		// IPreferenceStore preferenceStore = Activator.getDefault()
		// .getPreferenceStore();
		...
		// e.printStackTrace();
		// }
		return loadSetting == null ? defaultXLSSetting(xls_code) : loadSetting;
	}
	...
	static public CompositeMap defaultXLSSetting(String xls_code) {
		...
		dat.addChild(createMap(
				"target_code", Messages.PreferencesSetting_7, "2")); //$NON-NLS-1$ //$NON-NLS-3$
		CompositeMap createMap = createMap(
				"data", Messages.PreferencesSetting_10, "3"); //$NON-NLS-1$ //$NON-NLS-3$
		// createMap.put("idx", "0");
		dat.addChild(createMap);
	...
	public static CompositeMap defautIDXMap(String xls_code) {
		...
		m.addChild(createMap(
				"xls_code", Messages.PreferencesSetting_14, xls_code)); //$NON-NLS-1$
		m.addChild(createMap(
				"org_code", Messages.PreferencesSetting_16, Formater.org_code)); //$NON-NLS-1$ //$NON-NLS-3$
		m.addChild(createMap(
				"area_code", Messages.PreferencesSetting_19, Formater.area_code)); //$NON-NLS-1$ //$NON-NLS-3$
		m.addChild(createMap(
				"data_type", Messages.PreferencesSetting_22, Formater.data_type)); //$NON-NLS-1$ //$NON-NLS-3$
		m.addChild(createMap(
				"currency_type", Messages.PreferencesSetting_25, Formater.currency_type)); //$NON-NLS-1$ //$NON-NLS-3$
		m.addChild(createMap(
				"unit", Messages.PreferencesSetting_28, Formater.unit)); //$NON-NLS-1$ //$NON-NLS-3$
		m.addChild(createMap(
				"b_data_flag", Messages.PreferencesSetting_31, Formater.b_data_flag)); //$NON-NLS-1$ //$NON-NLS-3$
		m.addChild(createMap(
				"num_type", Messages.PreferencesSetting_34, Formater.num_type)); //$NON-NLS-1$ //$NON-NLS-3$
		m.addChild(createMap(
				"std_org_code", Messages.PreferencesSetting_37, Formater.std_org_code)); //$NON-NLS-1$ //$NON-NLS-3$
