...
public class PatternConstructor {
	...
	public static String interpretReplaceEscapes(String replaceText,
		...
		return new ReplaceStringConstructor(lineDelim).interpretReplaceEscapes(
				replaceText, foundText);
	}
	...
	private static class ReplaceStringConstructor {
		...
		private int interpretReplaceEscape(final char ch, int i,
				StringBuffer buf, String replaceText, String foundText) {
			int length = replaceText.length();
			switch (ch) {
			case 'r':
				buf.append('\r');
			...
			case 'R': // see
				...
				break;
			/*
			 * \0 for octal is not supported in replace string, since it would
			 * conflict with capturing group \0, etc.
			 */
			case '0':
				buf.append('$').append(ch);
				/*
				 * See explanation in
				 * "Feature in java.util.regex.Matcher#replaceFirst(String)" in
				...
				if (i + 2 < length) {
					...
					try {
						parsedInt = Integer.parseInt(
								replaceText.substring(i + 1, i + 3), 16);
						if (parsedInt < 0)
			...
			case 'u':
				if (i + 4 < length) {
					...
					try {
						parsedInt = Integer.parseInt(
								replaceText.substring(i + 1, i + 5), 16);
						if (parsedInt < 0)
							throw new NumberFormatException();
					} catch (NumberFormatException e) {
			...
			case 'C':
				if (foundText.toUpperCase().equals(foundText)) // is whole match
																// upper-case?
					fRetainCaseMode = RC_UPPER;
				else if (foundText.toLowerCase().equals(foundText)) // is whole
																	// match
																	// lower-case?
					fRetainCaseMode = RC_LOWER;
				else if (Character.isUpperCase(foundText.charAt(0))) // is first
																		// character
																		// upper-case?
