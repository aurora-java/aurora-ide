...
import ext.org.eclipse.jdt.internal.ui.text.JavaWhitespaceDetector;

public class SqlJSourceScanner extends RuleBasedScanner {
	public final static String SQLJ_source = "__sql_java_content_";
	public final static String SQLJ_VAR = "__sql_java_var_";

	public SqlJSourceScanner(IColorManager manager) {
		IToken string = new Token(new TextAttribute(
				manager.getColor(IJavaColorConstants.JAVA_STRING)));
		IToken sql_var = new Token(new TextAttribute(
				manager.getColor(IJavaColorConstants.JAVA_TYPE)));

		IToken sql_source = new Token(new TextAttribute(
		// JAVADOC_KEYWORD
				manager.getColor(IJavaColorConstants.TASK_TAG)));
		setDefaultReturnToken(sql_source);
		...
		/*
		 * On "high contrast" displays the default text color (black) is a
		 * problem, since the normal high contrast background is black.
		 * ("High contrast" is a Windows feature that helps vision-impaired
		 * people.) When high contrast mode is enabled, use different colors
		 * that show up better against a black background.
		 */
		...
		datatypeToken = new Token(new TextAttribute(
				manager.getColor(IJavaColorConstants.JAVA_KEYWORD
						+ PreferenceConstants.EDITOR_BOLD_SUFFIX)));
		functionToken = new Token(new TextAttribute(
				manager.getColor(IJavaColorConstants.JAVA_KEYWORD
						+ PreferenceConstants.EDITOR_BOLD_SUFFIX)));
		// identifierToken = new Token( new TextAttribute(
		// colorProvider.getColor( SQLColorProvider.SQL_HC_IDENTIFIER_COLOR )));
		delimitedIdentifierToken = new Token(new TextAttribute(
				manager.getColor(IJavaColorConstants.JAVA_STRING)));
		otherToken = new Token(new TextAttribute(
				manager.getColor(IJavaColorConstants.JAVA_STRING)));
		// }
		// else {
		// commentToken = new Token( new TextAttribute( colorProvider.getColor(
		// IColorConstants.SQL_COMMENT_COLOR )));
		// stringToken = new Token( new TextAttribute( colorProvider.getColor(
		// IColorConstants.SQL_QUOTED_LITERAL_COLOR )));
		// keywordToken = new Token( new TextAttribute( colorProvider.getColor(
		// IColorConstants.SQL_KEYWORD_COLOR )));
		// datatypeToken = new Token( new TextAttribute( colorProvider.getColor(
		// IColorConstants.SQL_KEYWORD_COLOR )));
		// functionToken = new Token( new TextAttribute( colorProvider.getColor(
		// IColorConstants.SQL_KEYWORD_COLOR )));
		// // identifierToken = new Token( new TextAttribute(
		// colorProvider.getColor( SQLColorProvider.SQL_IDENTIFIER_COLOR )));
		// delimitedIdentifierToken = new Token( new TextAttribute(
		// colorProvider.getColor(
		// IColorConstants.SQL_DELIMITED_IDENTIFIER_COLOR )));
		// otherToken = new Token( new TextAttribute( colorProvider.getColor(
		// IColorConstants.SQL_DEFAULT_COLOR )));
		// }
		...
		// rules[3] = new SqlJTagRule("#",sql_source);
		rules[2] = new SqlJTagRule("$", sql_var);

		rules[3] = new WhitespaceRule(new JavaWhitespaceDetector());
	...
	private static String[] fgKeywords = { "ABSOLUTE", //$NON-NLS-1$
			"ACQUIRE", //$NON-NLS-1$
			"ACTION", //$NON-NLS-1$
			"ADD", //$NON-NLS-1$
			"ALL", //$NON-NLS-1$
	...
	private static String[] fgDatatypes = { "INTEGER", //$NON-NLS-1$
	...
	private static String[] fgFunctions = { "ABS", //$NON-NLS-1$
			"ABSVAL", //$NON-NLS-1$
			"ACOS", //$NON-NLS-1$
			"ASCII", //$NON-NLS-1$
			"ASIN", //$NON-NLS-1$
	...
	/**
	 * Defines an array of arrays that combines all the SQL language elements
	 * defined previously.
	 */
	 ...
	 */
	public static List<String> getSQLAllWords() {
		List<String> words = new ArrayList<String>();
//		words.addAll(fgKeywords);
//		Collections
		for (String string : fgKeywords) {
			words.add(string);
		}
		for (String string : fgDatatypes) {
			words.add(string);
		}
		for (String string : fgFunctions) {
			words.add(string);
		}
		return words;
	}
