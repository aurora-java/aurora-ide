 ...
 *******************************************************************************/
package ext.org.eclipse.jdt.internal.ui.text;
 ...
 * single line comments, Java strings and Java characters.
 */
public class FastJavaPartitionScanner implements IPartitionTokenScanner,
	...
	private static final int Sql_J = 6;
	...
	private static final int CARRIAGE_RETURN = 6; // postfix for STRING,
													// CHARACTER and
													// SINGLE_LINE_COMMENT
	private static final int dollar = 7;// prefix for sqlj
	private static final int dollar_bracket = 8;// prefix for sqlj
	private static final int bracket = 9;// postfix for sqlj
	...
	private final IToken[] fTokens = new IToken[] { new Token(null),
			...
			new Token(JAVA_MULTI_LINE_COMMENT), new Token(JAVA_DOC),
			new Token(JAVA_CHARACTER), new Token(JAVA_STRING),
			SqlJSourceConfigration.SQLJ_TOKEN };
		...
		while (true) {
			...
			case JAVA:
				...
				case '{':
					if (fLast == dollar) {
						if (fTokenLength - getLastLength(fLast) > 0)
							return preFix(JAVA, Sql_J, dollar_bracket, 2);
						else {
							preFix(JAVA, Sql_J, dollar_bracket, 2);
							fTokenOffset += fTokenLength;
							fTokenLength = fPrefixLength;
							break;
						}
	...
	private static int getState(String contentType) {
		...
		else if (contentType.equals(SqlJSourceScanner.SQLJ_source))
			return Sql_J;
	...
	public void setPartialRange(IDocument document, int offset, int length,
		...
		if (offset == partitionOffset) {
			...
			if (Sql_J == fState) {
				fScanner.setRange(document, partitionOffset, length + offset
						- partitionOffset);
				fPrefixLength = 0;
				fState = JAVA;
			}
