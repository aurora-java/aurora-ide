 ...
 *******************************************************************************/
package patch.org.eclipse.jdt.ui.text;
...
import aurora.sql.java.editor.configration.SqlJSourceConfigration;
import aurora.sql.java.editor.configration.SqlJSourceScanner;

import patch.org.eclipse.jdt.internal.ui.JavaPlugin;
import ext.org.eclipse.jdt.internal.corext.util.CodeFormatterUtil;
import ext.org.eclipse.jdt.internal.ui.javaeditor.IClassFileEditorInput;
import ext.org.eclipse.jdt.internal.ui.javaeditor.ICompilationUnitDocumentProvider;
import ext.org.eclipse.jdt.internal.ui.javaeditor.JavaElementHyperlinkDeclaredTypeDetector;
import ext.org.eclipse.jdt.internal.ui.javaeditor.JavaElementHyperlinkDetector;
import ext.org.eclipse.jdt.internal.ui.javaeditor.JavaElementHyperlinkImplementationDetector;
import ext.org.eclipse.jdt.internal.ui.javaeditor.JavaElementHyperlinkReturnTypeDetector;
import ext.org.eclipse.jdt.internal.ui.javaeditor.JavaElementHyperlinkSuperImplementationDetector;
import ext.org.eclipse.jdt.internal.ui.javaeditor.NLSKeyHyperlinkDetector;
import ext.org.eclipse.jdt.internal.ui.text.AbstractJavaScanner;
import ext.org.eclipse.jdt.internal.ui.text.ContentAssistPreference;
import ext.org.eclipse.jdt.internal.ui.text.HTMLAnnotationHover;
import ext.org.eclipse.jdt.internal.ui.text.JavaCommentScanner;
import ext.org.eclipse.jdt.internal.ui.text.JavaCompositeReconcilingStrategy;
import ext.org.eclipse.jdt.internal.ui.text.JavaElementProvider;
import ext.org.eclipse.jdt.internal.ui.text.JavaOutlineInformationControl;
import ext.org.eclipse.jdt.internal.ui.text.JavaPresentationReconciler;
import ext.org.eclipse.jdt.internal.ui.text.JavaReconciler;
import ext.org.eclipse.jdt.internal.ui.text.PreferencesAdapter;
import ext.org.eclipse.jdt.internal.ui.text.SingleTokenJavaScanner;
import ext.org.eclipse.jdt.internal.ui.text.correction.JavaCorrectionAssistant;
import ext.org.eclipse.jdt.internal.ui.text.java.ContentAssistProcessor;
import ext.org.eclipse.jdt.internal.ui.text.java.JavaAutoIndentStrategy;
import ext.org.eclipse.jdt.internal.ui.text.java.JavaCodeScanner;
import ext.org.eclipse.jdt.internal.ui.text.java.JavaCompletionProcessor;
import ext.org.eclipse.jdt.internal.ui.text.java.JavaDoubleClickSelector;
import ext.org.eclipse.jdt.internal.ui.text.java.JavaFormattingStrategy;
import ext.org.eclipse.jdt.internal.ui.text.java.JavaStringAutoIndentStrategy;
import ext.org.eclipse.jdt.internal.ui.text.java.JavadocDoubleClickStrategy;
import ext.org.eclipse.jdt.internal.ui.text.java.PartitionDoubleClickSelector;
import ext.org.eclipse.jdt.internal.ui.text.java.SmartSemicolonAutoEditStrategy;
import ext.org.eclipse.jdt.internal.ui.text.java.hover.JavaEditorTextHoverDescriptor;
import ext.org.eclipse.jdt.internal.ui.text.java.hover.JavaEditorTextHoverProxy;
import ext.org.eclipse.jdt.internal.ui.text.java.hover.JavaInformationProvider;
import ext.org.eclipse.jdt.internal.ui.text.javadoc.JavaDocAutoIndentStrategy;
import ext.org.eclipse.jdt.internal.ui.text.javadoc.JavaDocScanner;
import ext.org.eclipse.jdt.internal.ui.text.javadoc.JavadocCompletionProcessor;
import ext.org.eclipse.jdt.internal.ui.typehierarchy.HierarchyInformationControl;
...
public class JavaSourceViewerConfiguration extends
	...
	private SqlJSourceConfigration ssc;
	...
	public JavaSourceViewerConfiguration(IColorManager colorManager,
		...
		initializeScanners();
		ssc = new SqlJSourceConfigration(editor,this.getColorManager());
	}
	...
	public JavaSourceViewerConfiguration(JavaTextTools tools, ITextEditor editor) {
		...
		fMultilineCommentScanner = (AbstractJavaScanner) fJavaTextTools
				.getMultilineCommentScanner();
		fSinglelineCommentScanner = (AbstractJavaScanner) fJavaTextTools
				.getSinglelineCommentScanner();
		fStringScanner = (AbstractJavaScanner) fJavaTextTools
				.getStringScanner();
		fJavaDocScanner = (AbstractJavaScanner) fJavaTextTools
				.getJavaDocScanner();
		fTextEditor = editor;
	}
	 ...
	 * <p>
	 * Clients are not allowed to call this method if the new setup without text
	 * tools is in use.
	 * </p>
	 * 
	...
	public IPresentationReconciler getPresentationReconciler(
		...
		ssc.connectPresentationReconciler(reconciler);
	...
	public IContentAssistant getContentAssistant(ISourceViewer sourceViewer) {
		...
		if (getEditor() != null) {
			...
			ContentAssistProcessor singleLineProcessor = new JavaCompletionProcessor(
					getEditor(), assistant,
					IJavaPartitions.JAVA_SINGLE_LINE_COMMENT);
			assistant.setContentAssistProcessor(singleLineProcessor,
					IJavaPartitions.JAVA_SINGLE_LINE_COMMENT);

			ContentAssistProcessor stringProcessor = new JavaCompletionProcessor(
					getEditor(), assistant, IJavaPartitions.JAVA_STRING);
			...
			ssc.installContentAssistant(assistant, sourceViewer);
	...
	public IQuickAssistAssistant getQuickAssistAssistant(
			ISourceViewer sourceViewer) {
		if (getEditor() != null) {
			JavaCorrectionAssistant assistant = new JavaCorrectionAssistant(
					getEditor());
			assistant
					.setRestoreCompletionProposalSize(getSettings("quick_assist_proposal_size")); //$NON-NLS-1$
			return assistant;
		}
		return null;
	}
		...
		if (IJavaPartitions.JAVA_DOC.equals(contentType)
			...
			return new IAutoEditStrategy[] {
					...
					new JavaAutoIndentStrategy(partitioning, getProject(),
							sourceViewer) };
		else if(SqlJSourceScanner.SQLJ_source.equals(contentType)||SqlJSourceScanner.SQLJ_VAR.equals(contentType)){
			return null;
		}
		else
	...
	public int[] getConfiguredTextHoverStateMasks(ISourceViewer sourceViewer,
			String contentType) {
		JavaEditorTextHoverDescriptor[] hoverDescs = JavaPlugin.getDefault()
	...
	public String[] getConfiguredContentTypes(ISourceViewer sourceViewer) {
		String[] exsitTypes = new String[] { IDocument.DEFAULT_CONTENT_TYPE,
				IJavaPartitions.JAVA_DOC,
				IJavaPartitions.JAVA_MULTI_LINE_COMMENT,
				IJavaPartitions.JAVA_SINGLE_LINE_COMMENT,
				IJavaPartitions.JAVA_STRING, IJavaPartitions.JAVA_CHARACTER };
		return ssc.getConfiguredContentTypes(exsitTypes, sourceViewer);
	}
	...
	private IDialogSettings getSettings(String sectionName) {
		...
		if (settings == null)
			settings = JavaPlugin.getDefault().getDialogSettings()
	...
	public IInformationPresenter getHierarchyPresenter(
		...
		if (getEditor() != null
				...
				&& JavaUI.getEditorInputJavaElement(getEditor()
						.getEditorInput()) == null)
			return null;
	 ...
	 */
	@Override
	protected Map<String, ITextEditor> getHyperlinkDetectorTargets(
			ISourceViewer sourceViewer) {
		Map<String, ITextEditor> targets = super
	...
	public IHyperlinkDetector[] getHyperlinkDetectors(ISourceViewer sourceViewer) {
		...
		List<IHyperlinkDetector> lists = new ArrayList<IHyperlinkDetector>();

		NLSKeyHyperlinkDetector nlsKeyHyperlinkDetector = new NLSKeyHyperlinkDetector();
		nlsKeyHyperlinkDetector.setContext(fTextEditor);
		lists.add(nlsKeyHyperlinkDetector);
		JavaElementHyperlinkDetector javaElementHyperlinkDetector = new JavaElementHyperlinkDetector();
		javaElementHyperlinkDetector.setContext(fTextEditor);
		lists.add(javaElementHyperlinkDetector);
		JavaElementHyperlinkImplementationDetector javaElementHyperlinkImplementationDetector = new JavaElementHyperlinkImplementationDetector();
		javaElementHyperlinkImplementationDetector.setContext(fTextEditor); 
		lists.add(javaElementHyperlinkImplementationDetector);
		JavaElementHyperlinkSuperImplementationDetector javaElementHyperlinkSuperImplementationDetector = new JavaElementHyperlinkSuperImplementationDetector();
		javaElementHyperlinkSuperImplementationDetector.setContext(fTextEditor);
		lists.add(javaElementHyperlinkSuperImplementationDetector);
		JavaElementHyperlinkDeclaredTypeDetector javaElementHyperlinkDeclaredTypeDetector = new JavaElementHyperlinkDeclaredTypeDetector();
		javaElementHyperlinkDeclaredTypeDetector.setContext(fTextEditor);
		lists.add(javaElementHyperlinkDeclaredTypeDetector);
		JavaElementHyperlinkReturnTypeDetector javaElementHyperlinkReturnTypeDetector = new JavaElementHyperlinkReturnTypeDetector();
		javaElementHyperlinkReturnTypeDetector.setContext(fTextEditor);
		lists.add(javaElementHyperlinkReturnTypeDetector);

		return lists.toArray(new IHyperlinkDetector[lists.size()]);
	}
